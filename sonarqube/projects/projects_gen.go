...
type BulkDeleteRequest struct {
	AnalyzedBefore    string `form:"analyzedBefore,omitempty"`    // Filter the projects for which last analysis of any branch is older than the given date (exclusive).<br> Either a date (server timezone) or datetime can be provided.
	...
	Qualifiers        string `form:"qualifiers,omitempty"`        // Comma-separated list of component qualifiers. Filter the results with the specified qualifiers
}
...
type CreateRequest struct {
	MainBranch             string `form:"mainBranch,omitempty"`             // Key of the main branch of the project. If not provided, the default main branch key will be used.
	...
	Visibility             string `form:"visibility,omitempty"`             // Whether the created project should be visible to everyone, or only specific user/groups.<br/>If no visibility is specified, the default project visibility will be used.
}
...
type ExportFindingsRequest struct {
	Branch      string `form:"branch,omitempty"`      // Branch key. When not specified, if no Pull Request key is defined either, it will default to the main branch
	Project     string `form:"project,omitempty"`     // Project key
	PullRequest string `form:"pullRequest,omitempty"` // Pull Request key. When not specified, the branch data will be returned instead
}
...
type SearchRequest struct {
	AnalyzedBefore    string `form:"analyzedBefore,omitempty"`    // Filter the projects for which the last analysis of all branches are older than the given date (exclusive).<br> Either a date (server timezone) or datetime can be provided.
	OnProvisionedOnly string `form:"onProvisionedOnly,omitempty"` // Filter the projects that are provisioned
	Projects          string `form:"projects,omitempty"`          // Comma-separated list of project keys
...
type UpdateKeyRequest struct {
	From string `form:"from,omitempty"` // Project key
	To   string `form:"to,omitempty"`   // New project key
}
...
type UpdateVisibilityRequest struct {
	Project    string `form:"project,omitempty"`    // Project, application or portfolio key
