...
type AddGroupToTemplateRequest struct {
	GroupName    string `form:"groupName"`              // Group name or 'anyone' (case insensitive)
	Permission   string `form:"permission"`             // Permission<ul><li>Possible values for project permissions admin, codeviewer, issueadmin, securityhotspotadmin, scan, user</li></ul>
...
type BulkApplyTemplateRequest struct {
	AnalyzedBefore    string `form:"analyzedBefore,omitempty"`    // Since 6.6;Filter the projects for which last analysis is older than the given date (exclusive).<br> Either a date (server timezone) or datetime can be provided.
	OnProvisionedOnly string `form:"onProvisionedOnly,omitempty"` // Since 6.6;Filter the projects that are provisioned
	Projects          string `form:"projects,omitempty"`          // Since 6.6;Comma-separated list of project keys
