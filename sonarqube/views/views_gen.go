...
type AddApplicationRequest struct {
	Application string `form:"application,omitempty"` // Key of the application to be added
	Portfolio   string `form:"portfolio,omitempty"`   // Key of the portfolio where the application will be added
}
...
type AddApplicationBranchRequest struct {
	...
	Branch      string `form:"branch,omitempty"`      // Key of the branch
	Key         string `form:"key,omitempty"`         // Key of the portfolio
}
...
type AddPortfolioRequest struct {
	Portfolio string `form:"portfolio,omitempty"` // Key of the portfolio where a reference will be added
	Reference string `form:"reference,omitempty"` // Key of the portfolio to be added
}
...
type ApplicationsRequest struct {
	Portfolio string `form:"portfolio,omitempty"` // Key of the would-be parent portfolio
}
...
type CreateRequest struct {
	...
	Name        string `form:"name,omitempty"`        // Name for the new portfolio
	Parent      string `form:"parent,omitempty"`      // Key of the parent portfolio, when creating a sub portfolio
	Visibility  string `form:"visibility,omitempty"`  // Whether the created portfolio or application should be visible to everyone, or only specific user/groups.<br/>Only applies to root portfolios. If no visibility is specified, the default visibility will be used.
}

// DeleteRequest Delete a portfolio definition. <br/>Requires 'Administrator' permission on the portfolio.
type DeleteRequest struct {
	Key string `form:"key,omitempty"` // Portfolio key
}
...
type MoveRequest struct {
	Destination string `form:"destination,omitempty"` // Key of the destination portfolio
	Key         string `form:"key,omitempty"`         // Key of the portfolio to move
}
...
type RemoveApplicationRequest struct {
	Application string `form:"application,omitempty"` // Key of the application to be removed
	Portfolio   string `form:"portfolio,omitempty"`   // Portfolio key
}
...
type RemoveApplicationBranchRequest struct {
	Application string `form:"application,omitempty"` // Key of the project
	Branch      string `form:"branch,omitempty"`      // Key of the branch
	Key         string `form:"key,omitempty"`         // Key of the portfolio
}
...
type RemovePortfolioRequest struct {
	Portfolio string `form:"portfolio,omitempty"` // Portfolio key
	Reference string `form:"reference,omitempty"` // Key of the referenced portfolio to be removed
}
...
type SetManualModeRequest struct {
	Portfolio string `form:"portfolio,omitempty"` // Key of the portfolio or sub-portfolio to update
}
...
type SetRegexpModeRequest struct {
	...
	Portfolio string `form:"portfolio,omitempty"` // Key of the portfolio or sub-portfolio to update
	Regexp    string `form:"regexp,omitempty"`    // A valid regexp with respect to the JDK's ``java.util.regex.Pattern`` class
}
...
type SetTagsModeRequest struct {
	Branch    string `form:"branch,omitempty"`    // Selects a branch in all matched projects, instead of using their main branches
	Portfolio string `form:"portfolio,omitempty"` // Key of the portfolio or sub-portfolio to update
	Tags      string `form:"tags,omitempty"`      // Comma-separated list of tags. It's not possible to set nothing.
}
...
type ShowRequest struct {
	Key string `form:"key,omitempty"` // The key of the portfolio
}
...
type UpdateRequest struct {
	...
	Key         string `form:"key,omitempty"`         // Key of the portfolio to update
	Name        string `form:"name,omitempty"`        // New name for the portfolio
}
