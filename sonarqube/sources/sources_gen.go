...
type ScmRequest struct {
	CommitsByLine string `url:"commits_by_line,omitempty"` // Group lines by SCM commit if value is false, else display commits for each line, even if two consecutive lines relate to the same commit.
	From          string `url:"from,omitempty"`            // First line to return. Starts at 1
	Key           string `url:"key"`                       // File key
	To            string `url:"to,omitempty"`              // Last line to return (inclusive)
}
