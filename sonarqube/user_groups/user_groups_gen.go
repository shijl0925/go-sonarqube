...
type SearchRequest struct {
	F       string `url:"f,omitempty"`       // Comma-separated list of the fields to be returned in response. All the fields are returned by default.
	Managed string `url:"managed,omitempty"` // Since 10.0;Return managed or non-managed groups. Only available for managed instances, throws for non-managed instances.
	Q       string `url:"q,omitempty"`       // Limit search to names that contain the supplied string.
}
...
type UsersRequest struct {
	Name     string `url:"name"`               // Group name
